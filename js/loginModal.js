function concluirCadastro() {
  ...
  if (!nameUser || !email || !senha || !confirmarSenha) {
    ...
    alert('Sua senha não atende aos requisitos: deve conter caractere especial, letra maiúscula, letra minúscula e ter no mínimo seis caracteres.');
    ...
    const emailCadastrado = listUser.find(user => user.emailSet === email);
    const nomeUsuarioCadastrado = listUser.find(user => user.nomeSet === nameUser);

    if (emailCadastrado) {
      alert("Este e-mail já está cadastrado. Por favor, faça login.");
      window.location.href = 'http://localhost:5500/login.html'; // Redireciona para a página de login
    } else if (nomeUsuarioCadastrado) {
...
function handleCredentialResponse(response) {
  ...
  if (data.email) {
    // Verifique se o e-mail já está cadastrado
    let listUser = JSON.parse(localStorage.getItem('listUser') || '[]');
    const emailCadastrado = listUser.find(user => user.emailSet === data.email);
    ...
    if (emailCadastrado) {
      ...
      alert("Sua Senha Automática é: " + data.sub + ", não se esqueça de alterar!");
